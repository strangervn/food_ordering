...
import com.example.appfoodv2.Presenter.ICart;
...
public class CartModels {
    ...
    private ICart callback;
    private FirebaseFirestore db;
    public CartModels(){

    }

    public CartModels(String id, String id_sanpham, long soluong) {
        this.id = id;
        this.id_sanpham = id_sanpham;
        this.soluong = soluong;
    }

    public CartModels(String id_sanpham, long soluong) {
        this.id_sanpham = id_sanpham;
        this.soluong = soluong;
    }

    public CartModels(ICart callback){
        ...
        db.collection("GioHang").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
            ...
            public void onSuccess(@NonNull QuerySnapshot queryDocumentSnapshots) {
                ...
                if(queryDocumentSnapshots.size()!=0){
                    ...
                    CartModels hangModels = new CartModels(idsp,1);
                    db.collection("GioHang").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
    ...
    public void HandleThanhToan(String ngaydat, String diachi, String hoten, String sdt, String phuongthuc, long tongtien, ArrayList<ProductModels> arrayList) {
        ...
        db.collection("HoaDon")
            ...
            public void onComplete(@NonNull Task<DocumentReference> task) {
                if(task.isSuccessful()){
                    for(ProductModels productModels : arrayList){
                        HashMap<String,Object> map_chitiet = new HashMap<>();
                        map_chitiet.put("id_hoadon",task.getResult().getId());
                        map_chitiet.put("id_sanpham", productModels.getIdsp());
                        map_chitiet.put("soluong", productModels.getSoluong());
                        ...
                        db.collection("ChitietHoaDon").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
                            ...
                            public void onComplete(@NonNull Task<DocumentReference> task) {
                                if(task.isSuccessful()){
                                    db.collection("GioHang").document(FirebaseAuth.getInstance().getCurrentUser().getUid())
                                            .collection("ALL").document(productModels.getId()).delete().addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
                                        public void onComplete(@NonNull Task<Void> task) {
                                            if(task.isSuccessful()){
                                                callback.OnSucess();
